...
def mask_rcnn_loss(pred_mask_logits, pred_boundary_logits, instances, pred_mask_bo_logits, pred_boundary_logits_bo, 
                    use_i_mask=False, pred_a_mask_logits=None, use_justify_loss=False, c_iter=None):
    """
    ...
    if use_justify_loss and use_i_mask and new_gt_bo_bounds.shape[0] > 0 \
            and pred_a_mask_logits_gt!=None and c_iter >= 20000:
        pred_mask_prob = nn.Softmax2d()(pred_mask_logits_gt)
        pred_a_mask_prob = nn.Softmax2d()(pred_a_mask_logits_gt)
        pred_bo_mask_prob = nn.Softmax2d()(pred_bo_mask_logits)

        occluded_part_gt = gt_masks * (gt_masks == gt_bo_masks).float()
        pred_occluded_part_1 = pred_a_mask_prob - pred_mask_prob
        pred_occluded_part_2 = pred_a_mask_prob * pred_bo_mask_prob
        rec_loss = F.binary_cross_entropy_with_logits(pred_occluded_part_1, occluded_part_gt) +\
                   F.binary_cross_entropy_with_logits(pred_occluded_part_2, occluded_part_gt)

    return mask_loss, bo_mask_loss, bound_loss, bo_bound_loss, a_mask_loss, rec_loss
...
class MaskRCNNConvUpsampleHead(nn.Module):
    ...
    def forward(self, x):
        ...
        return mask, boundary, mask_bo, boundary_bo, mask_head_features
